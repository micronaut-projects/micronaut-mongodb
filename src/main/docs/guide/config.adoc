...
snippet::io.micronaut.mongodb.docs.NamedClientService[tags="imports,clazz", project-base="docs-examples/csfle"]

==== MongoDB Health Checks

When the `mongo-reactive` or `mongo-coroutine` module is activated, the corresponding api:configuration.mongo.reactive.health.MongoHealthIndicator[] or api:configuration.mongo.coroutine.health.CoroutineMongoHealthIndicator[] is activated, resulting in the `/health` endpoint and api:health.CurrentHealthStatus[] interface resolving the health of the MongoDB connection.
