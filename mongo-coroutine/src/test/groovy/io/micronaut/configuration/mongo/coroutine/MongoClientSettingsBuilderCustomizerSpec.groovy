...
class MongoClientSettingsBuilderCustomizerSpec extends Specification {
    private static final String SPEC_NAME = 'mongo-client-settings-builder-customizer-coroutine'

    void "test configure picks up custom MongoClientSettings builder customizers for default coroutine client"() {
        given:
    ...
    void "test configure picks up custom MongoClientSettings builder customizers for named coroutine client"() {
        given:
        ApplicationContext context = ApplicationContext.run(
                (MongoSettings.EMBEDDED): false,
